...
1. **Dual retrieval** — dense (embedding-based) and sparse (TF-IDF) search run
   in parallel over per-language Annoy indexes.
...
## Why Annoy Over FAISS / ScaNN?

| Criterion          | Annoy                     | FAISS / ScaNN              |
|--------------------|---------------------------|----------------------------|
| Dependencies       | Zero native extras        | C++ build toolchain        |
| Cosine similarity  | Native (angular metric)   | Requires normalisation step|
| Scale              | Up to ~1 M items          | Billions of vectors        |
| Operational cost   | Single static file        | Server process or GPU      |

For content catalogs in the range of thousands to a million items, Annoy is fast
enough and dramatically simpler to operate. FAISS or ScaNN become relevant only
when the catalog grows beyond that range or when sub-millisecond latency at very
...
```
                   ...
                   embed (dense)                      TF-IDF vectorise
                          │                                   │
                          ▼                                   ▼
                  ANN search (Annoy)               ANN search (Annoy)
...
only changes the vector values — the Annoy index must be rebuilt after any model
